---
来源: 美团技术团队
原标题: 用 Agent 评测思路管理 AI Coding —— 31 万行代码 AI 重构的实践
发布: 2026年5月7日
原始 URL: https://mp.weixin.qq.com/s/CTY5mdgKh6TmPrO6xsKhWQ
说明: 已剥离 HTML 噪声 + 图片转 mermaid/文本描述。本文为合成 skill 的可追溯源材料。
---

## 全文速读

当 90% 以上代码由 AI 生成，决定系统走向的不是谁写得更快，而是约束 AI 的能力。没有统一规范，AI 只会成倍放大混乱。本文基于 31 万行代码重构实践，分享我们如何用 Agent 评测思路管理 AI Coding —— 通过技术债梳理、建设 Rule、重构 SOP 和 Pre-PR 机制，把重构从高成本专项变成随迭代持续推进的日常动作。我们沉淀的关键经验：
...
- 三、重构时间线与执行路径
  ...
  - 阶段二：调研并制定 AI 友好的研发规范（2026 年 2 月底完成）
  - 阶段三：建立 SOP，"见缝插针"完成渐进式重构（2026 年 3 月 - 4 月）
- 四、总结
  - 我们沉淀的关键经验
  - 行动指南：如果你的团队也想落地
...
- **经验一**：用 Agent 评测思路管理 AI Coding。我们团队负责 Agent 评测业务，在实践中沉淀出一套核心标准对齐理念："人人对齐→人机对齐"。我们发现管理 AI Coding 的底层逻辑一模一样 —— 先让团队形成统一共识（人人对齐），再将共识固化为 AI 可执行的约束（人机对齐）。本质上，就是同一套方法论在两个领域的复用。
...
Agent 评测系统长期承载多个核心业务场景，它同时承担了数据生产、流程编排、质量控制与多人协作等复杂能力，业务复杂度和工程复杂度都很高。具体来看，我们面对的复杂性主要体现在三个维度：
...
一年左右的时间，团队成员规模增加较多，并且团队成员技术背景复杂，涵盖高并发、机器学习离线训练、管理后端开发以及实习生，复杂业务系统开发经验不足。在这样一个高人员流动和跨技术栈的背景下，再叠加 90% 以上代码由 AI 辅助编写这一事实，如果不建立硬性的底层架构规范，不同背景的同学各自用 AI Coding，系统必将以极快的速度产生不可控的腐化与新债。
...
```mermaid
flowchart LR
    P1["阶段一 · 2026.02<br/>定义问题<br/>（Top-Down 思路）"]
    P2["阶段二 · 2026.02<br/>制定 AI 友好规范<br/>（人人对齐 → 人机对齐）"]
    P3["阶段三 · 2026.03 – 04<br/>建立 SOP 渐进式重构"]
    A1["Action 1<br/>100% 借助 AI 完成<br/>工程分层与解耦重构"]
    A2["Action 2<br/>零排期重构 —— 借着业务<br/>需求渐进式重构业务模型"]
    A3["Action 3<br/>重构质量保证<br/>（AI CR + Pre-PR + 测试 SOP）"]
...
_（配图：人机分工示意图 —— AI 负责"看全"（定向扫描全部 P0/P1 性能隐患、状态管理、索引等技术债），人负责"判断什么问题最重要"（圈定方向、定优先级、做最终决策）。）_
...
- **标准对齐（人人对齐）**：需要 1 位强有力的角色拉齐产品、运营、算法、QA 等所有角色的评测标准 —— 1 个"独裁者"好过 10 个"民主者"。
...
_（配图：Fail State vs Success State —— 团队没有共识时，AI 把每个人的不同理解放大成混乱代码（Fail）；团队先达成共识、再用 Rule Engine 把共识喂给 AI，产出整齐一致的代码（Success）。核心：用好 AI 的前提，是先让人对齐。）_
...
_（配图：核心痛点解法 —— 将"纸面共识 / 团队经验"（wiki 文档、口口相传）转化为系统约束：① **AI Rules（Always-on）** 用于约束工程规范、统一 Prompt、统一编码风格；② **AI Skills（Contextual Load）** 用于按场景渐进式加载的领域知识。一两个用好 AI 的人的经验，被高质量地泛化到全组。）_
...
```mermaid
flowchart TB
    subgraph LAYERS["四层架构（依赖方向自上而下，启动方向自下而上）"]
        direction TB
        CLIENT["demo-client<br/>（接口层）"]
        STARTER["demo-starter<br/>（接入层）"]
        APP["demo-application<br/>（应用层）"]
        INFRA["demo-infrastructure<br/>（基础设施层）"]
    end
    ...
    CLIENT_R["接口层职责<br/>负责对外接口协议定义，即对外提供的服务 client 包。"]
    STARTER_R["接入层职责<br/>负责系统启动配置和业务接入入口（如接口层实现、消息消费、定时任务等）的接口协议适配、基础参数校验、请求转发、响应结果封装、异常处理等工作。"]
    APP_R["应用层职责<br/>负责系统业务实现基础设施层定义，串联内部领域或基础设施层能力，实现流程编排、聚合查询等工作流操作。"]
    INFRA_R["基础设施层职责<br/>负责业务外部调用，如数据库的 CRUD、搜索引擎、文件系统、分布式服务的 RPC 等，通过依赖倒置，实现业务核心逻辑与外部调用的解耦。"]
    ...
    classDef layer fill:#eef7ff,stroke:#4a90c2,stroke-width:1px,color:#000;
    classDef resp fill:#fffbe6,stroke:#d4a017,stroke-width:1px,color:#000,text-align:left;
    class CLIENT,STARTER,APP,INFRA layer;
    class CLIENT_R,STARTER_R,APP_R,INFRA_R resp;
```
...
| 重构前                                                                                                                                                                                                                                                                                                                                                                                                                                           | 重构后                                                                                                                                                                                                                                                                                                                                                           |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _src/main/java/.../agent.evaluation 包按需求建包：adapter / autoeval / common / convert / dao / dataset（内嵌 constant、controller、crane、dataframe、domain、enums、hbase、pojo、repository、request、service、strategy、thrift、util、vo）/ dynamiccard / evaluation / infra / infrastructure / labeling / middleware / pipeline / python / qualitycheck / sbs / template / workflow / ApplicationLoader ——多个职能混杂在同一包下，分层不清。_ | _src/main/java/.../agent.evaluation 重构为四层：**application**（constants / converter / enums / model / repository / service [accesscontrol / analysis / automation / dataset / errorcorrection / labeling / orchestration / workbench] / util）+ **common** + **infrastructure** + **starter** —— 按"职责分层 + 业务域子包"重新组织，每层职责清晰、依赖单向。_ |
...
1. **引入 Pre-PR（预审）机制**：
   - 提交代码前，要求 RD 必须先用 AI 审查代码进行多轮自查，修复所有 AI 能发现的问题（规范类、Bug 类、异常处理、一致性、可扩展性及性能问题等）。
...
_（配图：AI CR 漏斗示意 —— 上层"AI 极速代码生成（海量代码）"产出大量代码块，中间经过"AI Code Review（多轮自查 + 高阶 Judge Model + 多厂商交叉审查）"过滤，下层留给"人工 Code Review（少量、最具业务价值）"。要点：AI 极大地压缩了 CR 压力，把 AI Coding 的提效红利留在 CR 之前消化；人的注意力集中在"我们是否在正确的约束下解决正确的问题"。）_

**2. 调研取经，建立 AI 辅助测试用例生成规范**
...
| Step3 设计分组 | 最小化用例数量       | 审核分组合理性，补充业务特殊场景             | 用判定表方法"先拆后合"，自动生成最小 Case 组合         | 组合爆炸的场景下，AI 比人算得快且不易出错    |
...
> **工程师角色的根本转变**
>
> 在代码 90% 由大模型生成的时代，研发团队的核心工作重心必须发生转移：从单纯的"编写业务代码"，转向去设计、维护一个能让 AI 稳定、可靠产出代码的"工程环境"。
>
> 先人人对齐，再人机对齐。让更多的人从"写代码"，转向去做更多 AI 的基础设施建设。
...
欢迎大家在文末评论区分享自己对 AI Coding 的见解和实践经验，一起学习、交流、进步。
