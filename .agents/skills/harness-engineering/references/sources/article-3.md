---
来源: 腾讯云开发者
原标题: 从第一性原理思考 Agentic Engineering
发布: 2026年4月23日
原始 URL: https://mp.weixin.qq.com/s/77OguM-HsX8V9WvNLWvlSw
...
这条公理定义了人的本质约束。这是认知心理学的基本结论（工作记忆容量有限、注意力是零和资源、决策疲劳是真实现象），不因技术进步而改变。在 AI 时代，制约产出的主要因素不再是编码速度，而是工程师审查、判断和决策的能力上限。最优策略因此不是最大化 AI 的产出量，而是**最优化工程师认知带宽的分配**。
...
这两个边界的**交叉**产生了四种不同的知识状态，每种状态对应不同的最优协作策略——这就是**乔哈里窗（Johari Window）**模型在人机协作中的应用 [6]：
...
```mermaid
flowchart LR
    A[需求澄清<br/>引导者] --> B[方案设计<br/>协作者]
    B --> C[编码 & 测试<br/>执行者]
    A -. 盲区→潜能区 .-> A
    B -. 私有知识注入<br/>盲区→开放区 .-> B
    C -. spec/设计已就位<br/>开放区 .-> C
```

- **需求澄清阶段**：任务主要落在盲区（人类有业务意图但未结构化，AI 不了解私有知识）和潜能区（AI 的通用知识可以帮助发现遗漏）。AI 的最优角色是**引导者**——通过结构化的提问，帮助工程师将模糊意图显式化为 spec，并利用通用知识和注入的领域最佳实践发现人类未曾考虑到的边界条件、异常路径和隐含约束。
- **方案设计阶段**：spec 将私有知识显式注入后，任务从盲区移向开放区。AI 的角色升级为**协作者**——主动分析方案权衡、提出替代设计、检查与架构约束的一致性、识别可扩展性或性能隐患，人类做最终的设计决策。
- **编码与测试阶段**：有了 spec 和设计文档作为高质量上下文，任务大部分进入开放区。AI 的角色转变为**执行者**，可以承担更大的自主性。
...
flowchart LR
    A[犯错] --> B[诊断根因]
    B --> C[检索现有知识]
    C --> D[创建或更新 Rule/Skill]
    D --> E[预防复发]
    E -.-> A
```

它与实践 5 形成互补——实践 5 自上而下编码已有知识，实践 6 自下而上从错误中生长新知识——共同构成完整的知识生命周期。

> **验证**：覆盖"可持续演进"目标维度，以及假设 1、假设 2 的修正立场。

# 04 基于 Skill 的 Agentic Engineering 框架
...
大量 Skills 不会拖垮上下文窗口，因为框架采用**三层加载机制**：
...
```mermaid
flowchart TB
    W[Workflow<br/>主驱动链] --> BP[Best Practices]
    W --> S[Standards]
    W --> D[Docs]
    T[Troubleshooting<br/>独立触发]
    SR[Self-Refinement<br/>横跨所有模块] -.沉淀经验.-> BP
    SR -.沉淀经验.-> S
    SR -.沉淀经验.-> R[Rules]
    SR -. 监听错误信号 .- W
    SR -. 监听错误信号 .- T
...
Workflow 是主驱动链，在执行每个阶段时按需拉取 Best Practices、Standards 和 Docs 作为上下文；Troubleshooting 独立于 Workflow，在问题排查场景下直接触发；Self-Refinement 横跨所有模块——它监听协作过程中的错误信号，将提取的经验沉淀到 Best Practices、Standards 或 Rules 中，形成持续改进的闭环。
...
```mermaid
flowchart TD
    Start([0. 评估任务复杂度]) --> Simple{简单任务?}
    Simple -->|是<br/>单文件、局部修改、意图明确| Skip[跳过 spec 检查<br/>直接加载 Standards 后编码]
    Simple -->|否<br/>多文件、跨模块、涉及设计决策| Spec{1. spec.md<br/>是否存在?}
    Spec -->|否| Switch[停止编码<br/>切换到<br/>requirements-clarification Skill]
    Spec -->|是| Read[读取并确认理解]
    Read --> Sections{2. 是否包含<br/>必要章节?<br/>（目标、需求、设计）}
    Sections -->|缺失| Guide[引导用户补充]
    Sections -->|完整| Load[3. 加载项目 Standards<br/>（编码规范、模块约束）]
    Load --> Code[4. 开始编码<br/>分步执行、每步可审查]
```
...
- [6] 乔哈里窗（Johari Window）模型的人机协作应用
