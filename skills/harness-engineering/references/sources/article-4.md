---
来源: TRAE.ai / 咸鱼
原标题: 万字干货：理解 Harness Engineering，看这一篇就够了
发布: 2026年4月10日
原始 URL: https://mp.weixin.qq.com/s/MzB8-B00GVdJy22j42CRgg
...
stateDiagram-v2
    [*] --> Perception
    Perception --> Planning: 结构化上下文
    Planning --> Action: 决策/工具调用
    Action --> Feedback: 执行结果/观测
    Feedback --> Perception: 新一轮感知
    Feedback --> [*]: 任务完成
```
...
sequenceDiagram
    autonumber
    participant User as 用户/外部世界
    participant CM as Context Manager (Read)
    participant LLM as LLM 推理 (Eval Plan)
    participant CI as Call Interceptor (Eval Exec)
    participant Tool as Tool Executor
    participant FA as Feedback Assembler (Print)

    User->>CM: 输入/事件
    CM->>LLM: 结构化 Prompt
    LLM->>CI: 生成调用意图 (Function Call)
    CI->>Tool: 路由 + 超时/配额监控
    Tool-->>CI: 执行结果或异常
    CI->>FA: 原始观测
    FA->>CM: 注入下一轮上下文
    Note over CM,FA: Loop 直至达成目标或终止条件
```
...
sequenceDiagram
    ...
    Harness->>Harness: 1. Schema 序列化<br/>(JSON Schema → Prompt 文本)
    Harness->>LLM: 注入工具定义 + 任务
    LLM-->>Harness: 2. 触发生成<br/>(name + args 文本)
    Harness->>Harness: 3. 确定性反序列化<br/>(最脆弱环节)
    alt 反序列化失败
        Harness-->>LLM: 重试 / 回退到自然语言
    else 反序列化成功
        Harness->>Tool: 调用执行
        alt 执行失败
            Tool-->>Harness: 错误信息
            Harness->>Harness: 交互式补充 / 重规划
        else 执行成功
            Tool-->>Harness: 结果
        end
    end
    Harness->>LLM: 4. 观测注入<br/>(结果作为 observation 进入下一轮)
```
...
flowchart TB
    subgraph CP["控制平面 (Control Plane)"]
        L1c["L1 呈现与集成层<br/>API 网关、事件总线"]
        L2c["L2 任务与状态管理层<br/>任务调度器、工作流引擎"]
        L3c["L3 Agent 核心引擎层<br/>行为规划器 (Behavior Planner)<br/>上下文策略器 (Context Strategist)"]
        L4c["L4 执行与治理层<br/>策略引擎、资源管理器"]
    end
    subgraph DP["数据平面 (Data Plane)"]
        L1d["L1: 客户端 SDK / Webhook<br/>嵌入到具体业务产品中"]
        L2d["L2: 状态存储 (State Storage)<br/>任务上下文与检查点持久化"]
        L3d["L3: Agent Runtime 进程<br/>短期/长期记忆存储"]
        L4d["L4: 沙盒执行框架、工具库 (Tool Library)"]
    end
    CP -.决策与策略下发.-> DP
    DP -.状态与遥测上报.-> CP
```
...
```mermaid
flowchart LR
    A["1. 信息源收集<br/>用户问题 + 短期记忆<br/>+ 长期知识检索"] --> B["2. 相关性排序<br/>时间 / 语义相似度"]
    B --> C["3. 压缩与摘要<br/>低密度内容结构化提炼"]
    C --> D["4. 预算分配<br/>按类别分配 Token 额度"]
    D --> E["5. 模板组装<br/>[user_request] / [tool_output] ..."]
    E --> F[("最终 Prompt")]
```
